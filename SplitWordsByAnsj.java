...
public class SplitWordsByAnsj {
	...
	public static void main(String[] args) {
		...
		//String words2 = "中国是世界四大文明古国之一，有着悠久的历史，距今约5000年前，以中原地区为中心开始出现聚落组织进而成国家和朝代，后历经多次演变和朝代更迭，持续时间较长的朝代有夏、商、周、汉、晋、唐、宋、元、明、清等。中原王朝历史上不断与北方游牧民族交往、征战，众多民族融合成为中华民族。20世纪初辛亥革命后，中国的君主政体退出历史舞台，取而代之的是共和政体。1949年中华人民共和国成立后，在中国大陆建立了人民代表大会制度的政体。中国有着多彩的民俗文化，传统艺术形式有诗词、戏曲、书法和国画等，春节、元宵、清明、端午、中秋、重阳等是中国重要的传统节日。";
		//System.out.println(ToAnalysis.parse(words2));
		File file = new File("C:/Users/hzzhangchi/Desktop/14.txt");
        String words = txt2String(file);
		//String words2 = "全国最大、最专业的软件联盟，1.本站上的所有软件和资料是软件作者提供、网友推荐及编辑人员收集整理的，如有侵犯你版权的，请指出我们将立即改正。2.本站所提供的软件我们都会尽可能测试再上传，但限于个人能力及系统等问题，无法保证所有软件都没有任何问题，也请自行对所下载的软件进行病毒、木马的防护查杀等手段。3.未经本站许可，任何人不得将本站网站网页的进行仿造。";
		//System.out.println(ToAnalysis.parse(words));
		List<Term> al = ToAnalysis.parse(words);
		//Object[] o = al.toArray();
		//System.out.println(o[1]=="是");
		//System.out.println(o[1]);
		//Term a = "zhangchi";
		//List<String> chi= new ArrayList<String>();
		//String[] zhang = null;
		String[] zhang = new String[al.size()];
		int j = 0;
		//a.add(h);
		...
		for(Iterator<Term> i = al.iterator();i.hasNext(); ){
			...
			*/
			if(j <= al.size()){
				zhang[j] = i.next().toString();
				//System.out.println(zhang[j]);
			}
			else{
				i.next();
			}
			...
			//System.out.println(contains(file,i.next().toString()));
			//chi.add(i.next().toString());
			j = j + 1;
		...
		for(int i=0;i<al.size();i++){
			if(contains(Richard,zhang[i])){}
			else{
				if (map.containsKey(zhang[i])){
					map.put(zhang[i],map.get(zhang[i])+1); 
				}
				else{
				map.put(zhang[i], 1); 
				}
		...
		for(int i=0;i<al.size();i++){
			if(contains(Richard,zhang[i])){}
				...
				if (map.containsKey(zhang[i])){
					map.put(zhang[i], Integer.parseInt(map.get(zhang[i]).toString())+1); 
				}
				else{
				map.put(zhang[i], 1); 
				}
		...
		*/
		//System.out.println(map);
		//String g = map.get(zhang[2]).toString();
